...
import type { LangfusePluginConfig, TargetConfig } from "./langfuse-exporter.js";
...
function activate(api: OpenClawPluginApi): void {
  ...
  const getOrCreateContext = (
    ...
    if (!activeCtx) {
      ...
      if (config.debug) {
        api.logger.info(
          `[Langfuse] NEW TraceContext: hook=${hookName}, channelId=${channelId}, runId=${effectiveRunId}, traceId=${activeCtx.traceId}`
        );
      }
    } else if (config.debug) {
      api.logger.info(
        `[Langfuse] REUSING TraceContext: hook=${hookName}, channelId=${channelId}, traceId=${activeCtx.traceId}`
      );
    }
  ...
  if (shouldHookEnabled("llm_output")) {
    api.on("llm_output", async (event: any, hookCtx: any) => {
      ...
      if (config.debug) {
        api.logger.info(
          `[Langfuse] Exported LLM span: ${event.provider}/${event.model}, duration=${now - startTime}ms`
        );
      }
    });
  }
  ...
  if (shouldHookEnabled("after_tool_call")) {
    api.on("after_tool_call", async (event: any, _hookCtx: any) => {
      ...
      if (config.debug) {
        api.logger.info(
          `[Langfuse] Exported tool span: ${toolName}, duration=${now - toolStartTime}ms`
        );
      }
    });
  }
      ...
      if (rootCtx.rootSpanStartTime || pendingAgentSpanId) {
        ...
        setTimeout(async () => {
          ...
          if (pendingAgentSpanId && agentEndAttrs) {
            ...
            if (config.debug) {
              api.logger.info(
                `[Langfuse] Ended agent span: spanId=${pendingAgentSpanId}, duration=${event.durationMs}ms`
              );
            }
          }
          ...
          if (rootSpanStartTime) {
            ...
            if (config.debug) {
              api.logger.info(
                `[Langfuse] Ended root span: spanId=${rootSpanId}, duration=${endTime - rootSpanStartTime}ms, traceId=${traceId}`
              );
            }
          }
  ...
  api.logger.info(
    `[Langfuse] Plugin activated (baseUrl: ${config.baseUrl || "cloud.langfuse.com"})`
  );
}
